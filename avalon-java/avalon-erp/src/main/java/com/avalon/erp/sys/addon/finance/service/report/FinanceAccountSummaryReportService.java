...
@Service
@Slf4j
public class FinanceAccountSummaryReportService extends AbstractReportService {
    @Override
    public String getServiceName() {
        return "finance.account.summary.report";
    }
    ...
    public ReportTable getReportTable(RecordRow recordRow) {
        ...
        // 构建科目汇总表数据
        Record summaryData = buildAccountSummaryData(accountSetIdValue, startPeriodStr, endPeriodStr);

        // 设置报表数据
        reportTable.setData(summaryData);
     ...
     * @return 科目汇总表数据
     */
    private Record buildAccountSummaryData(Integer accountSetId, String startPeriodStr, String endPeriodStr) {
        ...
        try {
            ...
            allAccounts.forEach(accountRow -> {
                ...
                // 创建科目记录
                RecordRow summaryRow = RecordRow.build();
                summaryRow.put("accountCode", accountCode);
                summaryRow.put("accountName", accountName);
                summaryRow.put("accountType", getAccountTypeEnum(accountType).getName());

                // 计算期初余额（包含借贷方向）
                BalanceInfo openingBalanceInfo = calculateOpeningBalanceWithDirection(accountCode, accountType,
                        openingBalanceData, beforeStartPeriodData);
                summaryRow.put("openingDebitAmount", openingBalanceInfo.getDebitAmount());
                summaryRow.put("openingCreditAmount", openingBalanceInfo.getCreditAmount());
                summaryRow.put("openingBalance", openingBalanceInfo.getBalance());

                // 计算本期发生额（包含借贷方向）
                BalanceInfo currentPeriodInfo = calculateCurrentPeriodBalanceWithDirection(accountCode, accountType,
                        currentPeriodData);
                summaryRow.put("currentDebitAmount", currentPeriodInfo.getDebitAmount());
                summaryRow.put("currentCreditAmount", currentPeriodInfo.getCreditAmount());

                // 计算期末余额（包含借贷方向）
                BalanceInfo endingBalanceInfo = calculateEndingBalanceWithDirection(accountCode, accountType,
                        openingBalanceInfo, currentPeriodInfo);
                summaryRow.put("endingDebitAmount", endingBalanceInfo.getDebitAmount());
                summaryRow.put("endingCreditAmount", endingBalanceInfo.getCreditAmount());
                summaryRow.put("endingBalance", endingBalanceInfo.getBalance());

                result.add(summaryRow);
            });

            log.info("科目汇总表数据构建完成，科目数量: {}", result.size());

        } catch (Exception e) {
            log.error("构建科目汇总表数据失败: {}", e.getMessage(), e);
        }
     ...
     * @return 期初余额信息
     */
    private BalanceInfo calculateOpeningBalanceWithDirection(String accountCode, String accountType,
            Record openingBalanceData, Record beforeStartPeriodData) {
     ...
     * @return 本期发生额信息
     */
    private BalanceInfo calculateCurrentPeriodBalanceWithDirection(String accountCode, String accountType,
            Record currentPeriodData) {
        BigDecimal debitAmount = BigDecimal.ZERO;
        BigDecimal creditAmount = BigDecimal.ZERO;
        BigDecimal balance = BigDecimal.ZERO;
    ...
    private BalanceInfo calculateEndingBalanceWithDirection(String accountCode, String accountType,
        ...
        // 2. 根据科目类型计算期末余额
        BigDecimal totalBalance = calculateBalanceByAccountType(getAccountTypeEnum(accountType), totalDebitAmount,
                totalCreditAmount);
     ...
     * @return 计算后的余额
     */
    private BigDecimal calculateBalanceByAccountType(AccountTypeEnum accountType, BigDecimal debitAmount,
