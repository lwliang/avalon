...
<template>
  <div class="w-full h-full bg" style="background-image:url('/color4bg.png');background-size: cover">
    <div class="w-full flex justify-center py-12 ">
      <div class="w-[300px] p-4 bg-background">
        <div class="flex justify-center">
          <my-image width="auto" class="" src="/logo.png"></my-image>
        </div>
        ...
        <div class="py-2" v-if="login_error">
          <div class="bg-danger-light-9 text-danger px-4 py-2 rounded">错误的登录名/密码</div>
        </div>
        <div class="pt-4">
          <MyButton type="primary" class="w-full" @click="loginClick" rounded>登录</MyButton>
        </div>
        <div class="flex justify-between pt-4">
          <my-text class="cursor-pointer" type="primary" @click="goRegisterClick">还没有账户?</my-text>
          <my-text class="cursor-pointer" type="primary" @click="goResetPasswordClick">重置密码</my-text>
        </div>
        <div class="pt-8">
          <MyDivider></MyDivider>
        </div>
        ...
        <div class="flex justify-center pt-4 h-auto items-center">
          <my-text class="cursor-pointer" type="primary" @click="goDatabaseManagerClick">管理数据库</my-text>
          <MyDivider direction="vertical"></MyDivider>
          <my-text class="cursor-pointer" type="primary">由Avalon提供支持</my-text>
        </div>
      </div>
    </div>
  </div>
