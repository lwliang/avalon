...
const emits = defineEmits(['expandChild', 'nodeClick', 'nodeSelect', 'nodeMove'])
...
<template>
    <div :class="[{'border':border}, 'w-full h-full p-2']">
        <template v-if="formField.value && formField.value.length">
            <draggable :list="formField.value" item-key="id"
                       group="trees" :sort="false"
                       @start="onDragStart"

                       @end="onDragEnd">
                <template #item="{ element }">
                    <my-tree-item :data="element" :nameField="nameField" :parentField="parentField"
                                  :childrenField="childrenField"
                                  :show-select="showSelect"
                                  :ref="setMyTreeRef"
                                  @selectValue="nodeSelect"
